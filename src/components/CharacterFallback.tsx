...
export function CharacterFallback({ basePosition }: Props) {
  ...
  useFrame((_, delta) => {
    ...
    if (phase === 'correct') {
      ...
      const p = t.current
      const targetX = selectedTileIndex >= 0 ? TILE_X[selectedTileIndex] : basePosition[0]
    ...
    if (phase === 'advancing') {
      scrolled.current = Math.min(scrolled.current + delta * SHIFT_SPEED, SHIFT_DIST)
      const ratio = scrolled.current / SHIFT_DIST
      pos.z = returnFrom.current[2] - scrolled.current
      pos.x = returnFrom.current[0] + (basePosition[0] - returnFrom.current[0]) * ratio
      pos.y = returnFrom.current[1] + (basePosition[1] - returnFrom.current[1]) * ratio
      return
    }
    ...
    if (phase === 'wrong') {
      ...
      pos.x = basePosition[0] + (targetX - basePosition[0]) * p
      pos.z = basePosition[2] + (ANSWER_ROW_Z - basePosition[2]) * Math.min(p * 2, 1)
